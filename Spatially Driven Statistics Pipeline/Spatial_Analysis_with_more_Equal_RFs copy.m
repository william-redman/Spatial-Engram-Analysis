function [ sim_activity_cell, rf_size_dist ] = Spatial_Analysis_with_more_Equal_RFs( animal_choices, activity_cell, ...
    correspondence_vec, n_mice, n_arms, n_days, days)
%% Spatial Analysis with more Equal RFs
%   This script is just like the original Spatial_Analysis.m function,
%   except that it tries to correct for unequal distributions of neurons
%   with RFs at baited vs. unbaited positions (see Methods for more
...
for ii = 1:n_mice
   ...
   maze_arm_pools = cell(1, n_arms); %neurons whose receptive field has been determined to be one of the eight maze arms
    ...
    for jj = 1:n_days
        ...
        total_size_of_pools = (available_neural_pool(1) - 1);
        for kk = 1:n_arms 
        % Finding the of neurons that are recruited and recalled
        % Note that it is here (at the expecation_recruited) that this
        % script is different from the previous script 
            if jj == 1
                expectation_recruited = n_recruited * visits(jj, kk, correspondence_vec(ii)) ...
                / sum(visits(jj, :, correspondence_vec(ii))); %old equation 
            else 
                expectation_recruited = (n_recruited / (n_arms - 1)) * (1 - length(maze_arm_pools{kk}) / ...
                    total_size_of_pools); %eq. 7
            end
